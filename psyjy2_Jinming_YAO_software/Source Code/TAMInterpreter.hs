        ...
        rtAux s = do
            ...
            case ms'' of
                ...
                Just s'' -> do
                    ...
                    rtAux $ switchThread s''

switchThread :: TAMState -> TAMState
switchThread s@(TAMState {tsID = id, tsPC = pc, tsLB = lb, tsST = st, tsTh = th}) = 
        if empty th then s
        else
            s { tsID = thID (head th),
                tsPC = thPC (head th),
                tsLB = thLB (head th),
                tsST = thST (head th),
                tsTh = tail th ++ [ThreadState{thID = id,thPC = pc, thLB = lb, thST = st}]}
    where
        empty [] = True
        empty _ = False
...
execute s@(TAMState {tsLB = lb, tsST = st, tsStk = stk}) i =
    case i of
        ...
        THREAD_START name   -> return Nothing
        -- update stack
        -- generate new pc st lb
        -- add it to th
        -- update pc
                    
        THREAD_CANCEL       -> return Nothing
        THREAD_WAIT name    -> return Nothing
        THREAD_LOCK name    -> return Nothing
        THREAD_UNLOCK       -> return Nothing
