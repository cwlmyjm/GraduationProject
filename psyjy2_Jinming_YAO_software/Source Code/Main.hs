...
-- import CTAMInterpreter
import RTAMInterpreter
import LibMT
        ...
        case ft of
            TAM -> do
                ...
                case mbCode of
                    ...
                    Just code ->
                        runTAM (optTrace opts) (optStep opts) (code ++ libMT)
            MT -> do
...
processOptions opts as = do
    ...
    where
        ...
        posAux opts aas@(a:as)
            | Prelude.take 2 a /= "--" = return (opts, aas)
            | otherwise        = do
                opts' <- poAux opts (Prelude.drop 2 a)
                posAux opts' as
        ...
        poAux opts o
            ...
            | o == "run-step" =
                return (opts {optRun = True, optTrace = True, optStep = True})
            | o == "version" =
