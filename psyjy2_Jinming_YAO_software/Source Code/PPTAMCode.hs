...
ppTAMInst i         = spcs cwLbl . pptiAux i
    where
        ...
        pptiAux (LOADCA l)   = ppOA "LOADCA"   l
        ...
        pptiAux (LOADLB m n) = ppOA "LOADLB"   (show m ++ " " ++ show n)
        pptiAux (LOADIB n)   = ppOA "LOADIB"   (show n)
        pptiAux (STOREIB n)  = ppOA "STOREIB"  (show n)
        ...
        pptiAux THREADSWITCHLOCK    = ppOA "THREADSWITCHLOCK" ""
        pptiAux THREADSWITCHUNLOCK  = ppOA "THREADSWITCHLOCK" ""
        pptiAux (THREADSTART l)      = ppOA "THREADSTART" l
        pptiAux THREADCANCEL         = ppOA "THREADCANCEL" ""
        pptiAux (THREADWAIT l)       = ppOA "THREADWAIT" l
        pptiAux (THREADLOCK l)       = ppOA "THREADLOCK" l
        pptiAux THREADUNLOCK         = ppOA "THREADUNLOCK" ""
        pptiAux THREADHALT           = ppOA "THREADHALT" ""
        pptiAux THREADSLEEP          = ppOA "THREADSLEEP" ""
...
fmtDisp d | d < 0     = "- " ++ show (abs ((fromIntegral d) :: Integer))
