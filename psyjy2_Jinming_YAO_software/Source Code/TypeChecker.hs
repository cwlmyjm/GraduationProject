...
chkCmd env (A.CmdThreadCancel {A.ctcId = c,A.cmdSrcPos = sp}) = do
    c' <- chkTpExp env c Integer
    return (CmdThreadCancel {ctcId = c',cmdSrcPos = sp})
chkCmd env (A.CmdThreadWait {A.ctwId = c,A.cmdSrcPos = sp}) = do
    c' <- chkTpExp env c Integer
    return (CmdThreadWait {ctwId = c',cmdSrcPos = sp})
