...
%token
    ...
    GETCHR      { (T_GETCHR, $$) }
    HALT        { (T_HALT, $$) }
    THREADSWITCHLOCK          { (T_THREADSWITCHLOCK,   $$) }
    THREADSWITCHUNLOCK        { (T_THREADSWITCHUNLOCK, $$) }
    THREADSTART               { (T_THREADSTART,        $$) }    
    THREADCANCEL              { (T_THREADCANCEL,       $$) }
    THREADWAIT                { (T_THREADWAIT,         $$) }
    THREADLOCK                { (T_THREADLOCK,         $$) }
    THREADUNLOCK              { (T_THREADUNLOCK,       $$) }
    THREADHALT                { (T_THREADHALT,         $$) }
    THREADSLEEP               { (T_THREADSLEEP,        $$) }
    LITINT      { (T_LitInt {}, _) }
    ID          { (T_Id {}, _) }
...
instruction : LOADL     litint          { LOADL $2 }
            ...
            | GETCHR                    { GETCHR }
            | HALT                      { HALT }
            | THREADSWITCHLOCK          { THREADSWITCHLOCK }
            | THREADSWITCHUNLOCK        { THREADSWITCHUNLOCK }
            | THREADSTART label         { THREADSTART $2 }    
            | THREADCANCEL              { THREADCANCEL }
            | THREADWAIT label          { THREADWAIT $2 }
            | THREADLOCK label          { THREADLOCK $2 }
            | THREADUNLOCK              { THREADUNLOCK }
            | THREADHALT                { THREADHALT }
