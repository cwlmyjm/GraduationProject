...
genCode optimize mtir = do
    ...
    return (if False then peepholeOpt code else code)
...
peepholeOpt' tis = poAux [] tis'
    where
        ...
        atAux lam tls (THREADSTART l : tis)      = atAux lam (l : tls) tis
        atAux lam tls (THREADWAIT l : tis)       = atAux lam (l : tls) tis
        atAux lam tls (THREADLOCK l : tis)       = atAux lam (l : tls) tis
