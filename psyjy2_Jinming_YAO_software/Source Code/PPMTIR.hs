...
ppCommand n (CmdLet {clDecls = ds, clBody = c, cmdSrcPos = sp}) =
    ...
    . ppCommand (n+1) c
ppCommand n (CmdThreadStart {ctsId = c,ctsCmds = cd,cmdSrcPos = sp}) = 
    indent n . showString "CmdThreadStart" . spc . ppSrcPos sp
    . nl
    . ppExpression (n+1) c
    . ppCommand (n+1) cd
ppCommand n (CmdThreadCancel {ctcId = c,cmdSrcPos = sp}) = 
    indent n . showString "CmdThreadCancel" . spc . ppSrcPos sp
    . nl
    . ppExpression (n+1) c
    
ppCommand n (CmdThreadWait {ctwId = c,cmdSrcPos = sp}) = 
    indent n . showString "CmdThreadWait" . spc . ppSrcPos sp
    . nl
    . ppExpression (n+1) c    

ppCommand n (CmdThreadLock {ctlLock = c,cmdSrcPos = sp}) = 
    indent n . showString "CmdThreadLock" . spc . ppSrcPos sp
    . nl
    . ppExpression (n+1) c   
    
ppCommand n (CmdThreadUnlock {ctuUnlock = c,cmdSrcPos = sp}) = 
    indent n . showString "CmdThreadUnlock" . spc . ppSrcPos sp
    . nl
    . ppExpression (n+1) c   
